...
void Casco_DMI::initDialog()
{
    ...
    dialshutdown->setGeometry(xpos+585,ypos+340,dialshutdown->width(),dialshutdown->height());
#endif
#ifndef Baseline_2_0
...
bool Casco_DMI::eventFilter(QObject *obj, QEvent *event)
{
        ...
        if((event->type()==QEvent::MouseButtonRelease))
        {
            if(isLogin)
            {
                ...
                if(dialelsmode->exec()==QDialog::Accepted)
                {
                    ...
                    refreshAlarmQue(29,"[车载] ",QString::number(i),true);
    ...
    else if( event->type()== QKeyEvent::KeyPress)
    {
        ...
        if(key->key()==Qt::Key_P&&key->modifiers()==Qt::CTRL)
        {
            //            if(!widDMS->isHidden())
...
void Casco_DMI::alarmPromte(QString txt)
{
    ...
    for(int i=0;i<list_alarm_display->size()&&i<5;i++)
    {
